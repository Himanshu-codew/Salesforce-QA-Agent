...
async def test_live_render_clean_now():
    async with httpx.AsyncClient(timeout=15.0) as http:
        # 1. Hit Render login
        r1 = await http.get("https://salesforce-qa-agent.onrender.com/api/auth/login?session_id=live_test_clean&domain=login", follow_redirects=False)
...
if __name__ == "__main__":
    asyncio.run(test_live_render_clean_now())
