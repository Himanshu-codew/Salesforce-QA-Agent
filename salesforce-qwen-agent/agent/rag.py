    ...
    def get_relevant_tools(self, user_query: str, top_k: int | None = None) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if not qset:
                ...
                logger.info("≡ƒöì RAG Retriever: Selected tools: []")
                return []
            ...
            # ΓöÇΓöÇ Read-only safety bias ΓöÇΓöÇ
            ...
            # ΓöÇΓöÇ Debug logging (never logs secrets/tokens) ΓöÇΓöÇ
            logger.debug(f"[RAG DEBUG] Query: {actual_query!r}")
            ...
            logger.info(f"≡ƒöì RAG Retriever: Selected tools: {selected_names}")
            return selected
