...
from .result_types import classify_result, render_sections
from .agent import (
...
def _render_required_fields_ask(tool_results: list[dict]) -> str | None:
    """Deterministic 'please provide the required fields' ask for blocked creates.

    PROBLEM 1 (create UX): when a vague "create a lead" is rejected by the
    fail-closed required-fields gate, the fields the user must supply are already
    resolved by the executor (live Describe metadata authoritative, static registry
    fallback; custom required fields included; only the user's own values count).
    Envelopes carrying ``missing_required=true`` hold exactly that list. When the
    whole response is a set of such fail-closed validation envelopes (no real data),
    render the ask deterministically here and SKIP the synthesizer LLM, so the bot
    always asks for the same fields instead of leaving the wording — and possible
    invented example values — to the model.

    Returns None when the result set cannot be answered this way (mixed with real
    tool data, or no required-fields envelope), so the normal synthesizer runs.
    """
    ...
    for item in tool_results:
        ...
        if not isinstance(parsed, dict):
            return None
        if parsed.get("validation_error") is not True or parsed.get("requires_user_input") is not True:
            return None
        envelopes.append(parsed)

    required_envs = [env for env in envelopes if env.get("missing_required") is True]
    ...
    if len(blocks) == 1:
        return blocks[0] + (
            "\n\nI won't create the record until you provide these values — "
            "I never fill them in myself. Share the exact values and I'll create it."
        )
    return "\n\n".join(blocks)
...
def _normalize_zero_count_result(tool_name: str, result: str, arguments: dict[str, Any]) -> str:
    ...
    synthesizer can state an attributed zero (e.g. "**Total Accounts: 0**")
    instead of treating a genuinely empty COUNT result as missing/unknown data.
...
def _split_reference_results(
    ...
    2. ``raw_remainder`` — everything the backend cannot render as a flat table
       ...
       results that Fix B already turned into an attributed ``**Total …: N**`` line) which
...
_METADATA_ONLY_TOOLS = {"getUserInfo", "getObjectSchema"}


def _try_render_deterministic_sections(tool_results: list[dict]) -> str | None:
    """Render labeled deterministic sections for a read-only turn.

    Fires when EVERY data-bearing result is classifiable (record list, single
    record, count, aggregate, empty):

    - Two or more data results → all sections rendered (labeled records, counts,
      empties, aggregates).
    - A single aggregate result → rendered as a deterministic aggregate table;
      the flat formatter must never collapse a GROUP BY / SUM into a count line
      or a record count.
    - A single record-list / count / empty result → falls through (returns
      None) so the existing byte-for-byte fast path (or the Fix B zero-count
      LLM path) is preserved.

    Results from related-record / schema / error / getUserInfo tools return
    None for classification, which falls through to the existing logic.
    """
    if not tool_results:
        return None
    data_items = [
        item
        for item in tool_results
        if isinstance(item, dict) and item.get("tool") not in _METADATA_ONLY_TOOLS
    ]
    if not data_items:
        return None
    infos = []
    for item in data_items:
        soql = item.get("query", "") if isinstance(item.get("query"), str) else ""
        info = classify_result(item.get("tool") or "soqlQuery", item.get("result", ""), soql)
        if info is None:
            return None
        infos.append(info)
    if len(infos) == 1 and infos[0].result_type != "aggregate":
        return None
    return render_sections(infos)
...
class Orchestrator:
    ...
    async def _stream_impl(
        ...
        for task in plan:
            ...
            for tc_idx, tc in enumerate(tool_calls):
                ...
                item = {"tool": tc_name, "result": normalized}
                if tc_name == "soqlQuery" and isinstance(tc_args, dict):
                    _q = tc_args.get("q") or tc_args.get("query")
                    if isinstance(_q, str) and _q.strip():
                        item["query"] = _q
                task_res.append(item)
    ...
    async def _synthesize_response(self, user_query: str, tool_results: list[dict]) -> str:
        # PROBLEM 1 (create UX): when a create was blocked by the required-fields
        # gate, the missing fields are deterministic (Describe/static registry).
        # Ask for them directly instead of delegating the wording to the LLM.
        deterministic_ask = _render_required_fields_ask(tool_results)
        if deterministic_ask is not None:
            logger.info(
                "[SYNTH] Blocked create detected; returning the deterministic "
                "required-fields ask (synthesizer LLM skipped)."
            )
            return deterministic_ask

        # Multi-result fidelity: when every data-bearing result in this turn is
        # classifiable, render ALL sections deterministically (labeled count /
        # record table / empty / aggregate) so a COUNT is never presented as a
        # record count and object labels (e.g. Leads) are always attributed.
        # This covers compound queries such as "show all Accounts AND tell me
        # how many Leads I have". Anything not classifiable (related records,
        # schemas, errors, getUserInfo) falls through to the existing paths.
        deterministic_sections = _try_render_deterministic_sections(tool_results)
        if deterministic_sections is not None:
            logger.info(
                "[SYNTH] Multi-result deterministic sections rendered; "
                "skipping Qwen synthesis."
            )
        ...
        metadata_only_tools = _METADATA_ONLY_TOOLS
        raw_tools = {
            item.get("tool")
